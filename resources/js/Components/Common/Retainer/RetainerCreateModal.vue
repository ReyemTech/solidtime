...
const { createRetainer } = useRetainersStore();
const show = defineModel('show', { default: false });
const saving = ref(false);

// — visible fields —
const name = ref('');
const periodUnit = ref<'weekly' | 'monthly' | 'quarterly'>('monthly');
const hoursPerPeriod = ref<number>(40);
const startsAt = ref('');

// — advanced fields —
const showAdvanced = ref(false);
const periodMode = ref<'calendar' | 'anchor' | 'explicit'>('calendar');
const anchorDate = ref('');
const billableOnly = ref(true);
const hardCapEnabled = ref(false);
const hardCapScope = ref<'per_period' | 'cumulative'>('per_period');
const hardCapEnforcement = ref<'block'>('block');
const cumulativeCapHours = ref<number>(0);
const subCapMode = ref<'soft' | 'strict'>('soft');
const endsAt = ref('');
...
async function submit() {
    ...
    try {
        ...
        const result = await createRetainer(payload);
        if (result) {
            emit('submitted');
            show.value = false;
        }
                ...
                <div v-if="showAdvanced" class="space-y-4 border-t border-border-secondary pt-4">
                    ...
                    <div v-if="hardCapEnabled" class="space-y-4 pl-4 border-l-2 border-border-secondary">
                        ...
                        <Field v-if="hardCapScope === 'cumulative'">
                            <FieldLabel for="cumulativeCapHours">Cumulative cap (hours)</FieldLabel>
                            <TextInput
